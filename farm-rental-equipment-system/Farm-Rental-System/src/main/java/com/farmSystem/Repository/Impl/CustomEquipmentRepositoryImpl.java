...
public class CustomEquipmentRepositoryImpl implements CustomEquipmentRepository {
	...
	public List<EquipmentDTO> serchEquipment(SearchEquipmentDTO searchEquipmentDTO) {
		...
		try(EntityManager entityManager = entityManagerFactory.createEntityManager()){
			...
			if(searchEquipmentDTO.getAvailability().toString() != null) {
				
				predicate = criteriaBuilder.and(predicate,criteriaBuilder
						.equal(root.get("availability"),searchEquipmentDTO.getAvailability().toString()));
			}
			...
			if(searchEquipmentDTO.getPageNumber() != 0 || searchEquipmentDTO.getPageSize() != 0) {
				 result = entityManager.createQuery(criteriaQuery)
						.setMaxResults(searchEquipmentDTO.getPageSize()).setFirstResult(skipSize).getResultList();
			}
			else {
				 result = entityManager.createQuery(criteriaQuery)
						.getResultList();
			...
			return result.stream().map(equipment -> equipmentMapper.equipmentEntityToEquipmentDTO(equipment)).collect(Collectors.toList());
		}
